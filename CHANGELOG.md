...
- `linear_elim` default-off retained after PLDI corpus differential (`chat/linelim_off.tsv` / `chat/linelim_on.tsv`): +3.3% total, 0 verdict regressions, 7 fixtures ≥ 200 ms regression (BinSum / BinSub 80×, EdDSA family 1.3–1.6×, Pedersen +2.9 s), zero clean wins. Docstring on `RuntimeConfig::linear_elim` records the measurement.
