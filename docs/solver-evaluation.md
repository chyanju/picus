...
- **Performance gap on dense-ideal circuits**: the 17-bench KPI suite (60 s timeout) currently solves 12/17, vs cvc5's 16/17. The 5 timeouts in the current build are `Pedersen@pedersen.r1cs` (cvc5 also times out on this one), `modulusagainst2p.r1cs`, `inTest.r1cs`, `chunkedadd1.r1cs`, and `test-rollup-tx-states.r1cs`. The first three are dense-ideal problems where naive sequential Buchberger hits its combinatorial wall regardless of pair-pruning heuristics; the last two regressed in 1.7.10 because the newly added Gebauer-Möller M-criterion's per-generation walk overhead is not yet amortized by a companion B-criterion (`myApplyBCriterion` in CoCoA), which is the natural next step. The literature solutions for the dense-ideal cases are F4/F5 (batched Macaulay-matrix reduction) or Montgomery-form arithmetic for the BN128 prime; both are out of scope. This is a performance limitation, not a correctness issue — all circuits that the native solver does resolve produce results identical to cvc5 (110/0-mismatch on the 110-circuit correctness gate).
