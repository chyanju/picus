...
- **Non-trivial UNSAT core tracing (`ffTraceGb`)**: cvc5's `Tracer` class hooks into CoCoA's Buchberger callbacks (`sPoly`, `reductionStart`, `reductionStep`, `reductionEnd`) to build a dependency DAG of polynomial derivations, then performs BFS backtrace to find the minimal input subset that produced the trivial ideal (UNSAT proof). Our `picus-solver` currently uses trivial mode (return all input facts). The feanor-math `ComputationController` trait only provides `checkpoint()` and `log()` — no S-polynomial or reduction-level callbacks. Implementing a non-trivial tracer would require either: (a) extending feanor-math's observer API with Buchberger-step callbacks, (b) implementing a custom Buchberger algorithm with built-in tracing, or (c) approximating the core by comparing pre/post GB sets. This is deferred to avoid maintaining a fork of feanor-math.
