...
//!   tightened a range, or pushed a learned constraint).
//! - Constraints pushed to `ctx.learned` are folded into the IR only at
//!   the *end* of an outer iteration, so within a single iteration
//!   every lemma sees the same IR snapshot. Inter-lemma ordering is
...
/// Per-iteration mutable state visible to every lemma. `known` and
/// `unknown` partition the wires; `ranges` carries finite-set
/// constraints (e.g. `{0, 1}` after binary01 fires); `learned` is the
/// out-buffer for new polynomial equalities.
