...
fn expected(p: &BigUint, vals: &[i64]) -> Vec<BigUint> {
    let mut v: Vec<BigUint> = vals.iter()
        .map(|&n| {
            if n >= 0 { BigUint::from(n as u64) % p }
            else {
                let nn = (-n) as u64;
...
fn test_distinct_roots_poly_small() {
    ...
    f3[4] = ff.one();
    assert_eq!(sorted_roots(&ff, &find_roots(&ff, &f3)), expected(&p, &[0, 1]));
    ...
    f4[6] = ff.one();                                // +x^6
    assert_eq!(sorted_roots(&ff, &find_roots(&ff, &f4)), expected(&p, &[0, 1]));
}
...
fn test_roots_full_small() {
    ...
    f1[2] = ff.one();
    assert_eq!(sorted_roots(&ff, &find_roots(&ff, &f1)), expected(&p, &[0, 1]));
    ...
    f2[4] = ff.one();
    assert_eq!(sorted_roots(&ff, &find_roots(&ff, &f2)), expected(&p, &[0, 1]));
    ...
    f5[2] = ff.one();
    assert_eq!(sorted_roots(&ff, &find_roots(&ff, &f5)), expected(&p, &[3, 5]));
...
fn test_distinct_roots_poly_big() {
    ...
    // f = x   →  {0}
    let f1 = vec![ff.zero(), ff.one()];
    assert_eq!(sorted_roots(&ff, &find_roots(&ff, &f1)), vec![BigUint::from(0u32)]);

    // f = x^3   →  {0}
    let f2 = vec![ff.zero(), ff.zero(), ff.zero(), ff.one()];
    assert_eq!(sorted_roots(&ff, &find_roots(&ff, &f2)), vec![BigUint::from(0u32)]);
