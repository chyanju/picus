...
impl PolyRingFacade {
    ...
    // ─── Pattern-matching helpers used by the propagation layer ──────

    /// Total degree of `p` = `max_t (sum_i exponent_t[i])`. Empty
    /// polynomial reports `0`.
    pub fn total_degree(&self, p: &Poly) -> usize {
        let mut best = 0usize;
        for (_, m) in self.terms(p) {
            let d: usize = (0..self.n_vars()).map(|i| self.exponent_at(&m, i)).sum();
            if d > best {
                best = d;
            }
        }
    ...
    /// Whether every term of `p` has total degree ≤ 1 (constants and
    /// linear terms only).
    pub fn is_linear(&self, p: &Poly) -> bool {
        self.total_degree(p) <= 1
    }

    /// Sole variable appearing in a univariate polynomial, if any.
    /// Returns `None` for constants and for polynomials in more than
    /// one variable.
    pub fn sole_variable(&self, p: &Poly) -> Option<usize> {
        let vars = self.appearing_indeterminates(p);
        if vars.len() == 1 {
            Some(vars.get(0).0)
        } else {
            None
    ...
    /// `p` as `coeff * x_i + const`, if linear univariate. Returns
    /// `(var_idx, lin_coeff, const_coeff)` when it matches; `None`
    /// otherwise. Both coefficients are field elements, reduced mod p.
    pub fn as_linear_univariate(&self, p: &Poly) -> Option<(usize, FfEl, FfEl)> {
        if !self.is_linear(p) {
            return None;
        }
        let var = self.sole_variable(p)?;
        let mut lin = self.ctx.field.zero();
        let mut cst = self.ctx.field.zero();
        for (c, m) in self.terms(p) {
            let total: usize = (0..self.n_vars()).map(|i| self.exponent_at(&m, i)).sum();
            match total {
                0 => cst = self.ctx.field.clone_el(c),
                1 if self.exponent_at(&m, var) == 1 => lin = self.ctx.field.clone_el(c),
                _ => return None,
            }
        }
        Some((var, lin, cst))
    ...
    /// Variable index of the leading monomial under DegRevLex. Returns
    /// `None` for the zero polynomial.
    pub fn leading_var(&self, p: &Poly) -> Option<usize> {
        let (_, m) = self.terms(p).next()?;
        for i in 0..self.n_vars() {
            if self.exponent_at(&m, i) > 0 {
                return Some(i);
            }
        }
        None
